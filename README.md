...
## Project Overview
Morpheus is a personal finance companion built with Flutter. It combines local-first UX (SQLite caching, fast UI) with Firebase cloud sync for accounts, cards, and expenses. Sensitive fields are encrypted in-app before they are written to Firestore. The app also supports card payment reminders, device app lock, and dashboard summaries.
...
### 6) Initialize Firebase configs in the project
Run FlutterFire to generate local config:
```bash
flutterfire configure
```
This updates `firebase.json` and `lib/firebase_options.dart`.
...
### 8) Deploy functions (for push reminders)
```bash
cd functions
npm install
...
flutter run
```
Open the app once so it stores your timezone and registers FCM tokens.
